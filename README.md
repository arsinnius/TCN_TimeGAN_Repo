...
**NOTE!! These calculations are for illustration only!!!** In reality, after perhaps many rounds of generating, evaluating  and tweaking the code, the user may wish to create thousands (millions) of synthetic time series to be used for training an AI/ML model. The simple format of this notebook would have to be modified considerably. Luckily, the Jupyter notebook format is flexible. For example, replace the "Samples to be Generated" cell and, rather than storing samples in the samples folder, write them to an `h5` file and only print every *n*th sample to indicate progress.
<br><br>
